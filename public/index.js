...
window.addEventListener('DOMContentLoaded', () => {
    ...
    axios.get(' http://18.144.6.133:3000/products').then((data) => {
        console.log(data);
...
function addToCart(productId) {
    axios.post(' http://18.144.6.133:3000/cart', { productId: productId })
        .then(response => {
...
function getCartDetails() {
    axios.get(' http://18.144.6.133:3000/cart')
        .then(response => {
            if (response.status === 200) {
...
document.addEventListener('click', (e) => {
    ...
    if (e.target.innerText == 'REMOVE') {
        axios.post(' http://18.144.6.133:3000/cart-delete-item', { 'productId': event.target.parentNode.parentNode.id.substring(8) })
            .then(response => {
    ...
    if (e.target.className == 'purchase-btn') {
        if (parseInt(document.querySelector('.cart-number').innerText) === 0) {
            ...
            axios.post(' http://18.144.6.133:3000/order-place')
                .then((response) => {
                    notifyUsers(response.data.message);
                    const cart_items = document.querySelector('#cart .cart-items');
...
function deleteCartItem(e, prodId) {
    e.preventDefault();
    axios.post(' http://18.144.6.133:3000/cart-delete-item', { productId: prodId })
...
function displayProducts(queryParams = '') {
    axios.get(` http://18.144.6.133:3000/products/${queryParams}`)
        .then(result => {
...
function displayInCart(queryParams = '') {
    axios.get(` http://18.144.6.133:3000/cart/${queryParams}`)
        .then(response => {
            const cart_items = document.querySelector('#cart .cart-items');
            cart_items.innerHTML = '';
